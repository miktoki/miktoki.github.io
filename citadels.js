...
class CitadelsData {
	ch = [
		...
		new Character("Cardinal", [`+1 card per ${blueSquare}`, `Can take remaining ${coin}s needed to buy ${district} from a player, must return cards equal to ${coin}s taken`], 5),
		/*6:*/
		new Character("Merchant", [`+1 ${coin} per ${greenSquare}`, `+1 ${coin}`], 6),
		new Character("Alchemist", [`Gain all ${coin}s paid to build ${district}`], 6),
		new Character("Trader", [`+1 ${coin} per ${greenSquare}`, `${hammer}${greenSquare}${district}x∞`], 6),
		/*7:*/
		...
		/*8:*/
		new Character("Warlord", [`+1 ${coin} per ${redSquare}`, `${bomb}${district}, pay 1 less ${coin} than its building cost`], 8),
		new Character("Marshal", [`+1 ${coin} per ${redSquare}`, `Seize ${district} with cost 3 or less. Pay player building cost.`], 8),
		new Character("Diplomat", [`+1 ${coin} per ${redSquare}`, `${rotate}${district} in opponents city with a ${district} in own city. Pay player difference in ${coin}`], 8),
		/*9:*/
		new Character("Queen", [`+3 ${coin}s if sitting next to rank 4 ${onePerson}`], 9, 5),
		new Character("Artist", [`Beautify up to two ${district} by adding 1 ${coin} on top of ${district} from own stash. ${district} cost +1 ${coin}. ${district} can only be beautified once.`], 9, 5),
	...
	ud = [
		...
		new UniqueDistrict("Basilica", `At the end of the game, score 1 extra point for each ${district} in your city with an odd-numbered cost.`, 4),
		new UniqueDistrict("Capitol", `If you have at least 3 ${district}s of the same type at the end of the game, score 3 extra points.`, 5),
		...
		new UniqueDistrict("Ivory Tower", `If the Ivory Tower is the only ${purpleSquare}${district} in your city at the end of the game, score 5 extra points.`, 5),
		...
		new UniqueDistrict("Wishing Well", `At the end of the game score 1 extra point for each ${purpleSquare}${district} in your city (including Wishing Well).`, 5),
	]
